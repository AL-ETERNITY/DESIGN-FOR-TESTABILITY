library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity datapath is
    port(
         clk, reset  : in std_logic;
         scan_enable : in std_logic;
         scan_instruction : in std_logic_vector(31 downto 0);
         PCSrc       : in std_logic;
         ALUSrc      : in std_logic;
         ALUControl  : in std_logic_vector(2 downto 0);
         RegWrite    : in std_logic;
         ImmSrc      : in std_logic_vector(1 downto 0);
         Instr       : buffer std_logic_vector(31 downto 0);
         MemWrite    : in std_logic;
         ResultSrc   : in std_logic_vector(1 downto 0);
         Zero        : out std_logic;
         mips_output : out std_logic_vector(31 downto 0)
    );
end datapath;

architecture rtl of datapath is

    -- Simple opcode validity check for scanned instructions
    -- Returns true only for known legal base RISC-V opcodes
    function is_valid_opcode(instr : std_logic_vector(31 downto 0)) return boolean is
        variable op : std_logic_vector(6 downto 0);
    begin
        op := instr(6 downto 0);
        if    op = "0110011"  -- R-type (ADD, SUB, AND, OR, etc.)
           or op = "0010011"  -- I-type (ADDI, etc.)
           or op = "0000011"  -- Loads
           or op = "0100011"  -- Stores
           or op = "1100011"  -- Branches
           or op = "1101111"  -- JAL
           or op = "1100111"  -- JALR
        then
            return true;
        else
            return false;
        end if;
    end function;

    signal RD1                             : std_logic_vector(31 downto 0):= (others => '0');
    signal PCPlus4                         : std_logic_vector(31 downto 0):= (others => '0');
    signal PCTarget                        : std_logic_vector(31 downto 0):= (others => '0');
    signal PCNext                          : std_logic_vector(31 downto 0):= (others => '0');
    signal Result                          : std_logic_vector(31 downto 0):= (others => '0');
    signal SrcB                            : std_logic_vector(31 downto 0):= (others => '0');
    signal ReadData                        : std_logic_vector(31 downto 0):= (others => '0');
    signal PC_buf                          : std_logic_vector(31 downto 0);
    signal ALUResult, WriteData, ImmExt    : std_logic_vector(31 downto 0):= (others => '0');
    signal instr_mem_out                   : std_logic_vector(31 downto 0);
    signal scan_instr_safe                 : std_logic_vector(31 downto 0);
    
    begin

        inst_PCSrc : entity work.mux_2(rtl)
                    generic map(32)
                    port map(
                      port_in1   => PCPlus4,
                      port_in2   => PCTarget,
                      sel        => PCSrc,
                      port_out   => PCNext
                    );

        inst_register : entity work.pc(rtl)
                    port map (
                      PCNext => PCNext,
                      clk    => clk,
                      reset  => reset,
                      PC_cur => PC_buf
                    );

        inst_instruction_memory : entity work.instr_mem(rtl)
                    port map(
                      addr_instr => PC_buf,
                      rd_instr   => instr_mem_out
                    );

        -- When scan_enable=0, only use the scanned instruction if its opcode is valid.
        -- Otherwise, substitute a safe ADD: add x3,x1,x2 (0x002081B3).
        scan_instr_safe <= scan_instruction when is_valid_opcode(scan_instruction)
                           else x"002081B3";

        Instr <= scan_instr_safe when scan_enable = '0' else instr_mem_out;

        inst_PCPlus4 : entity work.adder(rtl)
                        generic map(32)
                        port map(
                          a_in   => PC_buf,
                          b_in   => x"00000004",
                          c_out  => PCPlus4
                        );

        inst_register_file : entity work.reg_file(rtl)
                        port map(
                          read_port_addr1  => Instr(19 downto 15),
                          read_port_addr2  => Instr(24 downto 20),
                          write_port_addr  => Instr(11 downto 7),
                          write_data       => Result,
                          write_en         => RegWrite,
                          clk              => clk,
                          reset            => reset,
                          read_data1       => RD1,
                          read_data2       => WriteData
                        );

        inst_extend : entity work.extend(rtl)
                        port map(
                          ImmSrc       => ImmSrc,
                          Instruction  => Instr(31 downto 7),
                          ImmExt       => ImmExt
                        );

        inst_PCTarget :  entity work.adder(rtl)
                        generic map(32)
                        port map(
                          a_in   => PC_buf,
                          b_in   => ImmExt,
                          c_out  => PCTarget
                        );

        inst_ALUSrc : entity work.mux_2(rtl)
                    generic map(32)
                    port map(
                      port_in1   => WriteData,
                      port_in2   => ImmExt,
                      sel        => ALUSrc,
                      port_out   => SrcB
                    );

        inst_ALU : entity work.ALU(rtl)
                    port map(
                      SrcA       => RD1,
                      SrcB       => SrcB,
                      ALUControl => ALUControl,
                      Zero       => Zero,
                      ALUResult  => ALUResult);

        inst_data_memory : entity work.data_memr(rtl)
                    port map(
                      addr_port  => ALUResult,
                      write_data => WriteData,
                      clk        => clk,
                      reset      => reset,
                      write_en   => MemWrite,
                      read_data  => ReadData
                    );

        inst_ResultSrc : entity work.mux_3(rtl)
                    generic map(32)
                    port map(
                      a        => ALUResult,
                      b        => ReadData,
                      c        => PCPlus4,
                      sel      => ResultSrc,
                      y        => Result
                    );

        mips_output <= ALUResult;

    end rtl;